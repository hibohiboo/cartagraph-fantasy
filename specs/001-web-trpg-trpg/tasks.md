...
### タスク17: WebWorker統合実装（MVP版）[S]
**タイプ**: 実装-統合 | **優先度**: クリティカル | **工数**: 2時間
```bash
# シンプルなWebWorker + WASM統合（MVP）
```
**MVP受入条件（シンプル版）**:
- [ ] WebWorker内でのWASMモジュール動的読み込み
- [ ] 基本メッセージ送受信（postMessage/onmessage）
- [ ] 基本WASM関数呼び出し（create_session, add_player, roll_dice）
- [ ] 最小限のエラーハンドリング
- [ ] 動作確認テスト（Game.tsx）
...
**日付**: 2025-09-29
**完了タスク**: 13/30 (43.3%)
**現在フェーズ**: フェーズ2 - コアドメイン実装完了、統合フェーズ開始
...
### 次のタスク
- **タスク17**: WebWorker統合実装 [逐次依存] - フェーズ2最後のタスク
