...
- シナリオ3の自動テスト → 手動検証で十分（シナリオ1,2パターン確立済み）

**MVP理由**:
- シナリオ1,2のPlaywright自動テストでE2Eフローを完全確立
- 既存実装 (タスク20-25) で基本機能は完成しており、動作確認が主目的
- シナリオ3は手動検証で十分（ダイス振り・イベントログは視覚的確認が重要）
- 2シナリオの自動テストで回帰テスト基盤を構築完了

**依存関係**: タスク19 ✅, タスク20 ✅, タスク21 ✅, タスク22 ✅, タスク23 ✅, タスク25 ✅
**必要テスト**: シナリオ1,2自動テスト ✅ + シナリオ3手動検証
**成果物**: 2シナリオ自動テスト完備、3シナリオ動作確認済みフロントエンド実装
