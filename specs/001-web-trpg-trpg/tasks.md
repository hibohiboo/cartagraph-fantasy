...
**受入条件**:
- [ ] GameSession集約: 最小ケースから段階的実装
  - [ ] セッション作成 (最小ケース)
  - [ ] プレイヤー追加 (基本ケース)
  - [ ] セッション開始 (制約付き)
- [ ] Character集約: TDDサイクルで段階実装
- [ ] ScenarioTemplate: 基本機能から段階実装
- [ ] Event Sourcing: シンプルなイベント適用から
- [ ] 各ステップで RED → GREEN → Refactor サイクル完了
